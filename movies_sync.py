...
password = os.getenv("NEO4J_PASSWORD", "anhhuy123") 
...
def get_movie(title):
    result = driver.execute_query(
        query("""
            ...
            COLLECT(
                [person.name, HEAD(SPLIT(TOLOWER(TYPE(r)), '_')),REDUCE(s="",role In r.roles | s + role + ',')]
            ) AS cast
            LIMIT 1
        """),
        title=title,
...
@app.route("/movie/<title>/unvote", methods=["POST"])
def unvote_in_movies(title):
    summary=driver.execute_query(
        query(""" 
              Match(m:Movie {title:$title})
              SET m.votes= coalesce(m.votes, 0) - 1;
    ...
    updates=summary.counters.properties_set 
    return Response(dumps({"updates": updates}), mimetype="application/json") 
@app.route("/movie/<title>/delete", methods=["POST"])
def delete_movie(title):
    ...
    delete = summary.counters.properties_set
    return Response(dumps({"delete": delete}), mimetype="application/json")
